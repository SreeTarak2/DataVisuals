...
This paper is framed as a \textbf{systems design and prototype contribution}.
We document the implemented architecture in the DataSage platform, describe
how subjective novelty is operationalized end-to-end, and present real
measurements from the working system: on a test dataset with 15 seeded beliefs,
the system filtered 54.8\% of generated insights as redundant while presenting
novel findings with mean novelty score 0.71 (vs. 0.16 for filtered insights).
Expert validation confirms the filtering logic behaves correctly. A longitudinal
user study with professional analysts is necessary before claiming impact on
...
What we \textit{don't} have yet is empirical validation with real analysts
over meaningful periods. The paper identifies what needs to happen next and
maps out the user study design required before strong claims about productivity
gains can be responsibly made.
...
% --- Figure 1: Chat Interface with Generated Insights ---
% The DataSage platform showing the chat interface where insights are
% being generated. Multiple insights are displayed with their confidence
% scores, demonstrating the breadth of the QUIS generation pipeline.
\begin{figure}[htbp]
  \centering
...
% --- Figure 2: Belief Graph Filtering in Action ---
% The Analyse tab showing the filtered insights after the Belief Graph
% has been applied. The interface shows labeled insights with confidence
% scores, highlighting which insights made it through the novelty filter
% and why others were held back.
\begin{figure}[htbp]
  \centering
  ...
  \caption{The Analyse tab with Belief Graph filtering applied. Each insight
           ...
           indicating its status. The 30\% confidence threshold visible across all
...
\begin{enumerate}
  ...
  entries), filtering becomes selective. Experts appreciated the noise
  reduction.
  ...
  Domain-specific embedding context would help but isn't implemented.
\end{enumerate}
...
We initially relied only on Semantic Surprisal, but ran into a real
failure case early on. Suppose the Belief Graph says ``Monthly revenue
is typically around \$2M.'' The system then reports ``Monthly revenue
dropped to \$1.3M last quarter.'' Embedding models see both as being
about revenue with similar numeric context, and assign them moderate
similarity (around 0.77)---too low to block. But this insight is
actually crucial: a 35\% revenue drop is alarming and actionable.

That's where Bayesian Surprise steps in. If the prior distribution over
monthly revenue captures typical month-to-month fluctuation, a 35\% drop
creates high KL divergence and pushes the novelty score above threshold
even though the embeddings aren't alarmed. This ``quantitative drift''
pattern matters for analysts. It's why we kept both components rather
than simplifying to embeddings alone.

\subsection{Limitations and Open Questions}

\textbf{Cold Start Problem.} A brand-new user with an empty Belief
Graph won't see any personalization benefit on day one---the system
just acts like vanilla QUIS. We try to help with document ingestion
(analyzing uploaded reports to seed the graph), but a user truly starting
from scratch won't feel the personalization value until they've built
up some history.
...
10\%'' scored 0.83 cosine similarity despite being about different
things entirely. Injecting domain context into the embedding pipeline
would help, but we haven't implemented it yet.
...
The core insight motivating this work is simple: insights that users
already know aren't useful, no matter how statistically sound. A
personalized system should filter findings based on what each analyst
knows, not just on global statistical measures. That's what Subjective
Novelty Detection attempts.

We've implemented the SND framework as part of the DataSage platform:
a Belief Graph to model user knowledge, a hybrid scoring function
combining semantic and Bayesian surprise, and a five-node LangGraph
orchestrating the analysis pipeline with a Critic loop for self-correction.
The system is integrated, runs without crashing, and responds correctly to
domain expert feedback on what should and shouldn't be filtered.

Where we stand: The architecture is validated. The components work
together. What's missing is rigorous empirical validation with real
analysts over meaningful timeframes. That needs to happen before any
strong claims about productivity gains or satisfaction improvements.

The next steps are clear. We need a user study: 8--12 analysts using the
system on familiar data for 2--4 weeks, with metrics on how often they
mark insights as ``already knew it,'' and whether engagement improves.
We also need benchmark comparisons and robustness testing of the Belief
Graph (deliberately inject false beliefs to measure suppression rates).

Beyond that, a few extensions look promising. A reinforcement-learning
policy for the weight parameter $\alpha$ would adapt faster than the
exponential moving average we use now. Multimodal Belief Graphs---storing
embeddings of charts alongside text---would capture visual knowledge that
analysts build from dashboards. And team-level belief sharing could
compress the cold-start period for new hires. We plan to pursue at least
the first two in follow-on work.
...
invaluable---they caught edge cases we hadn't considered and validated
