...
\begin{itemize}
    ...
    three synthetic user profiles, an ablation study, a case study,
    and an automated evaluation harness that lets us re-score the
    full pipeline after every prompt change without recruiting
    annotators again (Section~\ref{sec:eval}).
\end{itemize}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      xlabel={Novelty threshold $\tau$},
      ylabel={Score},
      xmin=0.18, xmax=0.62,
      ymin=0.3, ymax=1.0,
      xtick={0.20,0.25,0.30,0.35,0.40,0.45,0.50,0.55,0.60},
    ...
    % Precision@10 (increasing)
    \addplot[blue, mark=square*] coordinates {
      (0.20,0.48) (0.25,0.56) (0.30,0.65) (0.35,0.75)
      (0.40,0.79) (0.45,0.81) (0.50,0.82) (0.55,0.83) (0.60,0.83)
    };
    \addlegendentry{Precision@10}
    % Recall@10 (decreasing)
    \addplot[red, mark=triangle*] coordinates {
      (0.20,0.96) (0.25,0.93) (0.30,0.89) (0.35,0.84)
      (0.40,0.74) (0.45,0.63) (0.50,0.51) (0.55,0.40) (0.60,0.32)
    };
    \addlegendentry{Recall@10}
    % Vertical dashed line at tau = 0.35
    \draw[dashed, black!70, thick] (axis cs:0.35,0.3) -- (axis cs:0.35,1.0);
    \node[font=\scriptsize, anchor=south west] at (axis cs:0.36,0.34) {$\tau\!=\!0.35$};
    \end{axis}
  \end{tikzpicture}
...
\begin{tabular}{@{}ll@{}}
  ...
  \texttt{source}     & \texttt{user\_dismissed} / \texttt{user\_accepted} / \texttt{document\_ingested} \\
...
\begin{itemize}
  ...
  this.'' This is the clearest signal we get, so we give it
  near-maximum confidence and add it right away.
  ...
  the same finding twice hints at partial familiarity. We set
  confidence lower at 0.70 because this channel is noisier---a
...
\subsection{Two-Layer Evaluation Protocol}

A fair assessment of a personalised insight system must answer two
distinct questions: (1)~does the novelty filter correctly suppress
what a given user already knows, and (2)~are the insights that
\emph{do} reach the user well-formed, grounded, and actionable?
Answering only the first validates the Belief Graph but ignores
presentation quality; answering only the second validates the
pipeline but tells us nothing about personalisation. We therefore
evaluate along both axes.

\subsubsection{Layer 1: Novelty Evaluation (Simulated Profiles)}

Three metrics target the novelty claim:
...
Krippendorff's $\alpha$ across annotators was 0.78---substantial
agreement for a task this subjective.

\subsubsection{Layer 2: End-to-End Quality (Automated Harness)}
\label{sec:autoeval}

We tried the obvious route first: recruit annotators, hand them
insights, collect scores. It worked for Layer~1 (fifteen profiles,
manageable scope), but it fell apart as soon as we started iterating
on prompts. Nobody wants to re-label hundreds of responses every
time you tweak a system message. So we built an automated harness
that runs the full pipeline end-to-end---chat query in, QUIS
analysis, Critic pass, Belief Graph filtering, KPI generation,
chart rendering, final response out---and scores everything
programmatically.

\textbf{Test runner.} A structured test suite of eight queries
(single-turn analytical, multi-turn conversational, and memory-chain
follow-ups) is dispatched against the live backend API for each
dataset. The runner auto-detects numeric, categorical, and temporal
columns and fills query templates with real column names, ensuring
that tests exercise the actual data rather than canned examples.
Each response is captured with full metadata: latency, chart
generation flag, conversation context, and raw text.

\textbf{Auto-scorer.} Responses are scored on five quality
dimensions (1--5 scale each):

\begin{itemize}
  \item \textit{Faithfulness}: grounded in actual query results;
  no fabricated numbers or deflected answers.
  \item \textit{Analytical Depth}: goes beyond surface-level
  commentary; provides multi-sentence reasoning.
  \item \textit{Specificity}: cites concrete values, segments,
  or caveats rather than vague qualifiers.
  \item \textit{Actionability}: suggests clear next decisions
  or follow-up questions.
  \item \textit{Format Quality}: proper Markdown structure,
  bolded metrics, separator-delimited follow-ups.
\end{itemize}

Scoring is deterministic: a catalogue of 13 failure patterns---each
defined by regex or heuristic checks with assigned severity
(critical / high / medium)---detects issues such as filler openings,
SQL error leakage, missing numeric evidence, hallucination markers,
and absent chart generation. Detected patterns lower the relevant
dimension score and are tallied across the run.

\textbf{Fix-plan generator.} After each evaluation run, the harness
produces a prioritised prompt-fix plan ranked by
$\text{severity} \times \text{hit count}$, identifying the weakest
quality dimension and the specific prompt function to edit. This
closes the loop: run evaluation $\to$ identify failures $\to$ fix
prompts $\to$ re-run. The entire cycle executes in minutes with no
human labelling.

\textbf{Why both layers?} They test different things, and skipping
either one leaves a blind spot. Layer~1 tells us whether SND
actually filters what the user already knows---the core claim of
this paper. Layer~2 tells us whether the rest of the system
(KPI generation, chart rendering, follow-up conversations) holds
together at production quality. You can imagine a system that
reliably spots novelty but writes terrible summaries, or one that
produces polished responses full of things the analyst already knew.
We needed to rule out both failure modes.

\subsection{Main Results}

\subsubsection{Layer 1: Novelty (Simulated Profiles)}
...
\subsubsection{Layer 2: End-to-End Quality (Automated)}

Table~\ref{tab:quality} reports the automated quality scores
averaged across the Retail and Banking datasets. After two
prompt-tuning iterations guided by the fix-plan generator, all
five dimensions reached acceptable levels ($\geq 4.0/5$). The
most impactful fixes targeted faithfulness (eliminating SQL error
leakage and answer deflection) and specificity (removing filler
openings and enforcing numeric citations).

\begin{table}[htbp]
  \centering
  \caption{Automated Quality Scores (1--5, after two tuning rounds)}
  \label{tab:quality}
  \begin{tabular}{@{}lcc@{}}
    \toprule
    ...
    Analytical Depth     & 3.8             & 4.3            \\
    Specificity          & 2.9             & 4.1            \\
    Actionability        & 3.5             & 4.2            \\
    Format Quality       & 3.4             & 4.4            \\
    \midrule
    \textbf{Overall}     & 3.4             & \textbf{4.3}   \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      ylabel={Precision@10},
      ymin=0, ymax=1.0,
      symbolic x coords={Novice, Returning, Expert},
      xtick=data,
    ...
    \addplot[fill=gray!40, draw=gray!70] coordinates {
      (Novice,0.33) (Returning,0.31) (Expert,0.30)
    };
    \addlegendentry{Pandas Prof.}
    % Linear QUIS
    \addplot[fill=blue!35, draw=blue!60] coordinates {
      (Novice,0.52) (Returning,0.50) (Expert,0.49)
    };
    \addlegendentry{Linear QUIS}
    % Sisu-style
    \addplot[fill=orange!40, draw=orange!70] coordinates {
      (Novice,0.49) (Returning,0.47) (Expert,0.46)
    };
    \addlegendentry{Sisu-style}
    % Agentic SND (ours)
    \addplot[fill=teal!50, draw=teal!80] coordinates {
      (Novice,0.58) (Returning,0.72) (Expert,0.80)
    };
    \addlegendentry{Agentic SND}
    \end{axis}
  \end{tikzpicture}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ybar interval,
      width=\columnwidth, height=4.8cm,
      xlabel={Hybrid Novelty Score $\mathcal{N}$},
      ylabel={Number of Insights},
      xmin=0, xmax=1,
      ymin=0, ymax=340,
    ...
    % Filtered (red) bins: 0.0–0.35
    \addplot[fill=red!40, draw=red!60] coordinates {
      (0.0,210) (0.05,310) (0.10,280) (0.15,190)
      (0.20,110) (0.25,60) (0.30,35) (0.35,0)
    };
    % Presented (green) bins: 0.35–1.0
    \addplot[fill=green!45, draw=green!65] coordinates {
      (0.35,25) (0.40,30) (0.45,20) (0.50,15)
      (0.55,18) (0.60,35) (0.65,75) (0.70,140)
      (0.75,195) (0.80,230) (0.85,160) (0.90,80)
      (0.95,30) (1.0,0)
    };
    % Vertical dashed line at tau = 0.35
    \draw[dashed, black!80, thick] (axis cs:0.35,0) -- (axis cs:0.35,340);
    \node[font=\scriptsize, anchor=south west, fill=white, inner sep=1pt]
      at (axis cs:0.36,300) {$\tau\!=\!0.35$};
    % Legend annotations
    \node[font=\scriptsize, red!70!black, anchor=east] at (axis cs:0.33,250) {filtered};
    \node[font=\scriptsize, green!50!black, anchor=west] at (axis cs:0.37,250) {presented};
    \end{axis}
  \end{tikzpicture}
...
\textbf{Synthetic evaluation.} The Precision@10 and satisfaction
numbers in Layer~1 were obtained with simulated user profiles, not
real analysts interacting with real datasets over time. Layer~2's
automated harness mitigates this partially---it runs the full
production pipeline end-to-end against live data, exercising KPIs,
charts, insights, and conversational follow-ups without synthetic
profiles---but it does not measure subjective novelty directly. A
longitudinal study with actual users remains necessary before drawing
stronger conclusions about production impact.
...
to 4.3 in satisfaction---are encouraging. Separately, an automated
harness that we run after every prompt change confirms that the
broader pipeline (KPIs, charts, follow-up conversations) has not
regressed---scores across faithfulness, depth, specificity,
actionability, and formatting all hold above our quality bar after
iterative tuning.
