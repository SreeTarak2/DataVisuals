...
class RAGService:
    ...
    async def _generate_data_slices(
        ...
        # Slice 2: Temporal trends (if temporal data exists)
        if query_intent["temporal_focus"]:
            temporal_cols = [col for col in df.columns if df[col].dtype == 'datetime64[ns]']
            if temporal_cols:
                slice_data = await self._create_temporal_slice(
                    df, temporal_cols[0], relevant_columns, query_intent
                )
                if slice_data:
                    slices.append(slice_data)
        
        # Slice 3: Distribution analysis
        if query_intent["query_type"] == "distribution_analysis":
            numeric_cols = [col for col in relevant_columns if col in df.columns and df[col].dtype in ['int64', 'float64']]
            if numeric_cols:
                slice_data = await self._create_distribution_slice(
                    df, numeric_cols[0], query_intent
                )
                if slice_data:
                    slices.append(slice_data)
        
        # Slice 4: Correlation analysis
        if query_intent["query_type"] == "comparative_analysis":
            numeric_cols = [col for col in relevant_columns if col in df.columns and df[col].dtype in ['int64', 'float64']]
            if len(numeric_cols) >= 2:
                slice_data = await self._create_correlation_slice(
                    df, numeric_cols[:2], query_intent
                )
                if slice_data:
                    slices.append(slice_data)
        
        # Slice 5: Sample data (always include)
    ...
    async def _create_temporal_slice(
        self, 
        df: pd.DataFrame, 
        temporal_col: str, 
        relevant_columns: List[str],
        ...
        """Create temporal slice for trend analysis."""
        try:
            # Find numeric column for values
            numeric_cols = [col for col in relevant_columns if col in df.columns and col != temporal_col and df[col].dtype in ['int64', 'float64']]
            if not numeric_cols:
                return None
            ...
            df['time_period'] = df[temporal_col].dt.to_period('M')  # Monthly grouping
            
            result = df.groupby('time_period')[value_col].sum().reset_index()
            ...
            return {
                ...
                "data": result.to_dict('records'),
                "columns": ['time_period', value_col],
                "row_count": len(result),
                "description": f"Time series data showing {value_col} trends"
            }
            
        except Exception as e:
            logger.error(f"Error creating temporal slice: {e}")
            return None
    
    async def _create_distribution_slice(
        self, 
        df: pd.DataFrame, 
        numeric_col: str, 
        query_intent: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        """Create distribution slice for numeric analysis."""
        try:
            ...
            bins = pd.cut(data, bins=10, precision=2)
            ...
            return {
                ...
                "columns": ['range', 'count'],
                "row_count": len(result),
                "description": f"Histogram showing distribution of {numeric_col}"
            }
            
        except Exception as e:
            logger.error(f"Error creating distribution slice: {e}")
            return None
    
    async def _create_correlation_slice(
        self, 
        df: pd.DataFrame, 
        numeric_cols: List[str], 
        query_intent: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        """Create correlation slice for comparative analysis."""
        try:
            if len(numeric_cols) < 2:
                return None
            ...
            # Sample data for scatter plot (limit to 100 points)
            sample_df = df[[col1, col2]].dropna().sample(n=min(100, len(df)), random_state=42)
            ...
            return {
                ...
                "description": f"Scatter plot data showing relationship between {col1} and {col2}"
            }
            
        except Exception as e:
            logger.error(f"Error creating correlation slice: {e}")
            return None
