...
class DataProfiler:
    ...
    def _analyze_column(series: pd.Series, col_name: str) -> Dict[str, Any]:
        ...
        col_info['is_categorical'] = series.dtype == 'object' and series.nunique() < len(series) * 0.8
    ...
    def _generate_summary_stats(df: pd.DataFrame) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'temporal_columns': int(len(temporal_cols)),
    ...
    def _assess_data_quality(df: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            'overall_score': float(round(quality_score, 2)),
            'missing_data_impact': float(round(null_percentage * 0.5, 2)),
            'duplicate_impact': float(round(duplicate_percentage * 0.3, 2)),
