...
class QueryExecutor:
    ...
    def _postprocess_interpretation(cls, text: str) -> str:
        ...
        confidence_rewrites = [
            ...
            (
                r"(?i)\bimpossible to evaluate\b",
                "not identifiable from the available variation",
            ),
        ]
        ...
        # BUT NOT in markdown tables (between | and |)
        lines = cleaned.split("\n")
        processed_lines = []
        for line in lines:
            # Skip markdown table rows (contain | with numbers)
            if "|" in line and line.strip().startswith("|"):
                processed_lines.append(line)
            else:
                # Only bold numbers outside tables
                line = re.sub(
                    r"(?<![\w*`])(\d[\d,]*(?:\.\d+)?%?)(?![\w*`])",
                    r"**\1**",
                    line,
    ...
    def _sanitize_sql(sql: str) -> str:
        ...
        if re.search(r"\)\s*UNION\s*\(", sql, re.IGNORECASE):
            order_by_match = re.search(
                r"ORDER\s+BY\s+(.+?)(?=\s*(?:LIMIT|OFFSET|WHERE|GROUP|HAVING|$))",
                sql,
                re.IGNORECASE | re.DOTALL,
            )
            if order_by_match:
                order_by_clause = order_by_match.group(0).strip()
                sql_without_order = re.sub(
                ...
                sql = f"SELECT * FROM (\n{sql_without_order}\n) AS union_result\n{order_by_clause}"
                logger.info(f"Fixed UNION ORDER BY: wrapped in subquery")
...
Table name: `data`
Columns and types:
{self._get_column_schema(df)}

Sample data (first 5 rows):
{self._get_sample_data(df)}
...
class QueryClassifier:
    ...
    # Patterns that can be answered from metadata (expanded)
    METADATA_PATTERNS = [
        # Metadata questions
        ...
        # Summary/analysis questions that don't need SQL (can use metadata context)
        r"^(summarize|summary|overview|summary of)\s+(the\s+)?(data|trends|patterns)",
        r"^(what are the|describe the)\s+(main\s+)?(trends|patterns|insights|findings)",
        r"^(tell me about|tell us about)\s+(the\s+)?(trends|patterns|insights)",
        r"\b(brief|quick|overall|general)\s+(summary|overview|idea)",
        r"\b(analysis|insights?|findings?)\s+(from|about|on)\s+(this|the)\s+(data|dataset)",
        # Interpretation questions (don't need SQL)
        r"^(what does|how do i interpret|explain)\s+",
        r"^(what do you think|your thoughts|your opinion)",
        r"\b(mean|tell me more|go on|continue)\b",
        # Clarification requests
        r"^(why|how come|what reason)",
        r"\b(can you|could you)\s+(explain|clarify|elaborate)",
    ]
    ...
    def needs_sql_execution(cls, query: str) -> bool:
        ...
        for pattern in cls.METADATA_PATTERNS:
            if re.search(pattern, query_lower):
                logger.debug(f"Query matched metadata pattern: {pattern}")
                return False

        # Check if SQL execution is needed - must be explicit data requests
        ...
        # This is more conservative and avoids unnecessary SQL queries
        logger.debug(f"Query defaulting to False (no SQL): {query[:50]}...")
        return False
