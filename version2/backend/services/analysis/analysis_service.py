...
class AnalysisService:
    ...
    def find_strong_correlations(
        self, df: pl.DataFrame, threshold: float = 0.7
    ) -> List[Dict[str, Any]]:
        results = []
    ...
    def find_spearman_correlations(
        self, df: pl.DataFrame, threshold: float = 0.7
    ) -> List[Dict[str, Any]]:
    ...
    def detect_outliers_iqr(
        self, df: pl.DataFrame, multiplier: float = 1.5
    ) -> List[Dict[str, Any]]:
        results = []
        numeric_cols = df.select(pl.col(self.numeric_dtypes)).columns
    ...
    def find_dominant_categories(
        self, df: pl.DataFrame, threshold: float = 0.5
    ) -> List[Dict[str, Any]]:
        results = []
    ...
    def analyze_distribution(self, df: pl.DataFrame) -> List[Dict[str, Any]]:
        ...
        for col in numeric_cols:
            ...
            if len(series) < 5:
                continue
            skewness = float(
                np.mean(((series - np.mean(series)) ** 3)) / (np.std(series) ** 3)
            )
            results.append(
                {
                    "type": "distribution",
    ...
    def t_test_between_groups(
        self, df: pl.DataFrame, group_col: str, value_col: str
    ) -> Dict[str, Any]:
        try:
    ...
    def chi_square_test(
        ...
        try:
            ...
            crosstab = pd.crosstab(pdf[col1], pdf[col2])
    ...
    def run_pca(self, df: pl.DataFrame, n_components: int = 2) -> Dict[str, Any]:
        try:
            ...
            if np.isnan(data).any():
                logger.info(
                    "Found NaN values in data, dropping rows with NaN values for PCA"
                )
                mask = ~np.isnan(data).any(axis=1)
                if mask.sum() < 5:
                    logger.warning(
                        "Not enough valid rows for PCA after removing NaN values"
                    )
                    return {}
                data = data[mask]
            ...
            if len(data) < n_components:
                logger.warning(
                    f"Not enough data points ({len(data)}) for {n_components} components"
                )
                return {}
    ...
    def run_kmeans(self, df: pl.DataFrame, n_clusters: int = 3) -> Dict[str, Any]:
        try:
            ...
            if np.isnan(data).any():
                logger.info(
                    "Found NaN values in data, dropping rows with NaN values for KMeans"
                )
                # Drop rows with any NaN values
                mask = ~np.isnan(data).any(axis=1)
                if mask.sum() < n_clusters * 2:  # Need at least 2 points per cluster
                    logger.warning(
                        "Not enough valid rows for KMeans after removing NaN values"
                    )
                    return {}
                data = data[mask]
            ...
            kmeans = KMeans(n_clusters=min(n_clusters, len(data)), random_state=42).fit(
                data
            )
            return {
                "type": "kmeans",
    ...
    def find_deep_insights(
        self, df: pl.DataFrame, max_depth: int = 2
    ) -> List[Dict[str, Any]]:
        """
        ...
        basic_correlations = self.find_strong_correlations(
            df, threshold=0.3
        ...
        category_patterns = self._find_category_specific_patterns(
            df, max_depth=max_depth
        )
        deep_insights.extend(category_patterns)

        # Step 4: Search for temporal patterns in subsets
        temporal_patterns = self._find_temporal_subspace_patterns(
            df, max_depth=max_depth
        )
        deep_insights.extend(temporal_patterns)

        logger.info(
            f"QUIS subspace search completed. Found {len(deep_insights)} deep insights."
        )
        return deep_insights

    def _search_correlation_subspaces(
        self, df: pl.DataFrame, correlation: Dict, max_depth: int
    ) -> List[Dict[str, Any]]:
        ...
        for cat_col in categorical_cols[
            :5
        ]:  # Limit to top 5 categorical columns for performance
            try:
                ...
                for value in unique_values[:5]:  # Limit to top 5 values per column
                    ...
                    if subspace_correlation is not None:
                        ...
                        if (
                            correlation_strength > base_correlation + 0.2
                        ):  # At least 0.2 improvement
                            insights.append(
                                {
                                    "type": "subspace_correlation",
                                    "base_insight": correlation,
                                    "subspace": {cat_col: value},
                                    "subspace_correlation": round(
                                        subspace_correlation, 3
                                    ),
                                    "improvement": round(
                                        correlation_strength - base_correlation, 3
                                    ),
        ...
        if max_depth >= 2 and len(categorical_cols) >= 2:
            insights.extend(
                self._search_two_level_subspaces(df, correlation, categorical_cols)
            )

        return insights

    def _search_two_level_subspaces(
        self, df: pl.DataFrame, correlation: Dict, categorical_cols: List[str]
    ) -> List[Dict[str, Any]]:
        ...
        for cat_col1, cat_col2 in combinations(
            categorical_cols[:3], 2
        ):  # Limit combinations
            try:
                ...
                for val1 in values1:
                    for val2 in values2:
                        ...
                        if subspace_correlation is not None:
                            ...
                            if (
                                correlation_strength > base_correlation + 0.3
                            ):  # Higher threshold for 2-level
                                insights.append(
                                    {
    ...
    def _find_category_specific_patterns(
        self, df: pl.DataFrame, max_depth: int
    ) -> List[Dict[str, Any]]:
            ...
            for num_col in numeric_cols[:3]:
                try:
                    ...
                    for value in unique_values[:5]:
                        ...
                        mean_deviation = (
                            abs(subspace_mean - overall_mean) / overall_std
                            if overall_std > 0
                            else 0
                        )
                        ...
                        if (
                            mean_deviation > 1.5
                        ):  # More than 1.5 standard deviations different
                            insights.append(
                                {
    ...
    def _find_temporal_subspace_patterns(
        self, df: pl.DataFrame, max_depth: int
    ) -> List[Dict[str, Any]]:
    ...
    def _calculate_trend_strength(
        self, df: pl.DataFrame, temp_col: str, num_col: str
    ) -> float:
        """Calculate trend strength using linear regression correlation."""
        try:
            df_with_numeric_time = df
    ...
    def run_quis_analysis(
        self, df: pl.DataFrame, dataset_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        quis_results = {
            ...
            "summary": {
                ...
                "high_significance_insights": len(
                    [i for i in deep_insights if i.get("significance") == "high"]
                ),
                "very_high_significance_insights": len(
                    [i for i in deep_insights if i.get("significance") == "very_high"]
                ),
            },
        }
    ...
    def get_correlation_matrix(self, df: pl.DataFrame) -> Dict[str, Any]:
        ...
        data_df = df.select(numeric_cols).drop_nulls()
        if len(data_df) < 5:
    ...
    def compare_distributions(
        self, df: pl.DataFrame, numeric_col: str, group_col: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            # Get unique groups
            clean_df = df.select([group_col, numeric_col]).drop_nulls()
            ...
            for group in top_groups:
                group_data = clean_df.filter(pl.col(group_col) == group)[
                    numeric_col
                ].to_numpy()
                if len(group_data) >= 5:
                    stats = {
            ...
            groups_data = [
                clean_df.filter(pl.col(group_col) == g)[numeric_col].to_numpy()
                for g in top_groups
                if len(clean_df.filter(pl.col(group_col) == g)) >= 5
            ]
    ...
    def analyze_distributions_comprehensive(
    ...
    def find_correlations_comprehensive(
        self,
        df: pl.DataFrame,
        threshold: float = 0.3,
    ...
    def compare_groups(
        self, df: pl.DataFrame, group_col: str, value_col: str, alpha: float = 0.05
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for group_val in unique_groups[:10]:  # Limit to 10 groups
                data = groups_df.filter(pl.col(group_col) == group_val)[
                    value_col
                ].to_numpy()
                if len(data) >= 3:
            ...
            if len(group_data) == 2:
                ...
                if use_parametric:
                    result = hypothesis_tester.welch_t_test(
                        g1_data, g2_data, alpha=alpha
                    )
                else:
                    result = hypothesis_tester.mann_whitney_u_test(
                        g1_data, g2_data, alpha=alpha
                    )
                ...
                return {
                    ...
                    "group_statistics": {
                        str(g1_name): {
                            "mean": round(float(np.mean(g1_data)), 4),
                            "std": round(float(np.std(g1_data)), 4),
                            "n": len(g1_data),
                        },
                        str(g2_name): {
                            "mean": round(float(np.mean(g2_data)), 4),
                            "std": round(float(np.std(g2_data)), 4),
                            "n": len(g2_data),
                        },
                    },
                }
    ...
    def analyze_time_series(
        self, df: pl.DataFrame, value_col: str, time_col: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if len(data) < 10:
                return {
                    "error": "Insufficient data for time series analysis (need >= 10 points)"
                }
            ...
            acf_result = time_series_analyzer.calculate_autocorrelation(
    ...
    def calculate_feature_importance(
        self, df: pl.DataFrame, target_col: str, is_classification: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            numeric_cols = [
                c
                for c in df.select(pl.col(self.numeric_dtypes)).columns
                if c != target_col
            ]
    ...
    def run_enhanced_analysis(
        self, df: pl.DataFrame, depth: str = "standard"
    ) -> Dict[str, Any]:
        """
        ...
        if full_row_count > SAMPLE_THRESHOLD:
            ...
            sampled = True
            logger.info(
                f"Sampled {SAMPLE_THRESHOLD:,} of {full_row_count:,} rows for statistical analysis"
            )
        ...
        results["correlations"] = self.find_correlations_comprehensive(
            df, threshold=0.4
        )
        ...
        if depth == "advanced":
            ...
            results["anomalies_ml"] = self.detect_anomalies_advanced(
