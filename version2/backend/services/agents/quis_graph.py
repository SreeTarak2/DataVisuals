...
async def planner_node(state: AgentState) -> Dict[str, Any]:
    ...
    """
    logger.info(
        f"[PLANNER] Starting question generation for dataset {state['dataset_id']}"
    )
    ...
    if not state["questions"]:
        try:
            ...
            # Dataset _id is stored as a UUID string; ObjectId is kept as legacy fallback.
            db = get_database()
            dataset_id = state["dataset_id"]
            dataset = await db.uploads.find_one(
                {"_id": dataset_id, "user_id": state["user_id"]}
            )
            if dataset is None:
                try:
                    ...
                    dataset = await db.uploads.find_one(
                        {"_id": ObjectId(dataset_id), "user_id": state["user_id"]}
                    )
                except Exception:
                    pass
            ...
            # Load data for schema analysis — use parquet_path if available, fall back to file_path
            ...
            try:
                ...
                logger.warning(
                    f"[PLANNER] Parquet read failed ({data_path}), falling back to CSV: {e}"
                )
...
async def analyst_node(state: AgentState) -> Dict[str, Any]:
    ...
    try:
        # Get dataset — _id is stored as UUID string
        db = get_database()
        dataset_id = state["dataset_id"]
        dataset = await db.uploads.find_one(
            {"_id": dataset_id, "user_id": state["user_id"]}
        ...
        if not dataset:
            return {
                "last_error": f"Dataset {dataset_id} not found",
                "iteration_count": state["iteration_count"] + 1,
            }
        ...
        try:
            ...
            logger.warning(
                f"[ANALYST] Parquet read failed ({data_path}), falling back to CSV: {e}"
            )
            df = pl.read_csv(data_path)
...
async def novelty_filter_node(state: AgentState) -> Dict[str, Any]:
    ...
    if numbers and isinstance(insight_dict, dict):
        ...
        if metric_value:
            ...
            bayesian_surprise = bayesian_tracker.update_prior(
...
async def viz_designer_node(state: AgentState) -> Dict[str, Any]:
    ...
    try:
        # Load dataset once for all visualizations — _id is stored as UUID string
        db = get_database()
        dataset_id = state["dataset_id"]
        dataset = await db.uploads.find_one(
            {"_id": dataset_id, "user_id": state["user_id"]}
        ...
        try:
            ...
            logger.warning(
                f"[VIZ] Parquet read failed ({data_path}), falling back to CSV: {e}"
            )
            df = pl.read_csv(data_path)
        ...
        for i, insight in enumerate(approved):
            try:
                ...
                if len(valid_columns) < 1:
                    logger.warning(
                        f"[VIZ] No valid columns for insight {i + 1}, skipping"
                    )
                    continue
                ...
                if traces:
                    ...
                    logger.info(
                        f"[VIZ] Generated {chart_type} chart for insight {i + 1}"
                    )

            except Exception as e:
                logger.warning(
                    f"[VIZ] Failed to generate chart for insight {i + 1}: {e}"
                )
                continue
    ...
    async for state in graph.astream(initial_state, config):
        ...
        for node_name, node_state in state.items():
            if isinstance(node_state, dict):
                logger.debug(
                    f"Node {node_name}: iteration {node_state.get('iteration_count', '?')}"
                )
