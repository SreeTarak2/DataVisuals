...
class FAISSVectorService:
    ...
    def _initialize_components(self):
        try:
            ...
            logger.info(
                f"Embedding model '{self.embedding_model_name}' loaded successfully"
            )
    ...
    def _initialize_faiss_indices(self):
        try:
            dataset_index_path = os.path.join(
                self.vector_db_path, "dataset_index.faiss"
            )
            dataset_metadata_path = os.path.join(
                self.vector_db_path, "dataset_metadata.pkl"
            )

            if os.path.exists(dataset_index_path) and os.path.exists(
                ...
                logger.info(
                    f"Loaded dataset index with {self.dataset_index.ntotal} vectors"
                )
            else:
            ...
            query_metadata_path = os.path.join(
                self.vector_db_path, "query_metadata.pkl"
            )
    ...
    async def add_query_to_history(
        self, query: str, dataset_id: str, user_id: str
    ) -> bool:
    ...
    async def get_vector_db_stats(self, user_id: str) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "indices": {
                "datasets": {
                    "total_vectors": self.dataset_index.ntotal
                    if self.dataset_index
                    else 0,
                    "user_vectors": len(
                        [
                            m
                            for m in self.dataset_metadata.values()
                ...
                "query_history": {
                    ...
                    "user_vectors": len(
                        [
                            m
                            for m in self.query_history_metadata.values()
                            if m["user_id"] == user_id
                        ]
                    ),
                },
    ...
    async def reset_vector_db(self, user_id: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Vector database reset for user {user_id} (lazy rebuild pending)"
            )
            return True
        ...
        try:
            if self.dataset_metadata:
                ...
                for metadata in self.dataset_metadata.values():
                    embedding = np.array(
                        self.embedding_model.embed_documents([metadata["content"]])
                    )
        ...
        try:
            if self.query_history_metadata:
                ...
                for metadata in self.query_history_metadata.values():
                    embedding = np.array(
                        [self.embedding_model.embed_query(metadata["query"])]
                    )
    ...
    async def delete_dataset_chunks(self, dataset_id: str, user_id: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Marked {len(indices_to_remove)} chunks for deletion from dataset {dataset_id}"
            )
            return True
    ...
    def _initialize_chunk_index(self):
        ...
        try:
            ...
            chunk_metadata_path = os.path.join(
                self.vector_db_path, "chunk_metadata.pkl"
            )
            ...
            if os.path.exists(chunk_index_path) and os.path.exists(chunk_metadata_path):
                ...
                logger.info(
                    f"Loaded chunk index with {self.chunk_index.ntotal} vectors"
                )
            else:
