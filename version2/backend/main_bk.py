...
from fastapi import FastAPI, HTTPException, UploadFile, File, Depends, status, Form, Query, WebSocket, WebSocketDisconnect
...
from services.datasets.vector_service import faiss_vector_service
...
async def chat_socket(websocket: WebSocket):
    ...
    if not token:
        await websocket.close(code=4401, reason="Missing authorization token")
        return
    ...
    try:
        while True:
            ...
            client_conversation_id = payload.get("clientConversationId")
            ...
            try:
                ...
                logger.error(f"WebSocket chat processing failed: {exc}")
                await websocket.send_json({
                    "type": "error",
                    "clientMessageId": client_message_id,
                    "status_code": 500,
