...
# Module-level DataFrame cache to avoid repeated disk reads
_DF_CACHE = {}
_DF_CACHE_MAX_SIZE = 5

QUIS_MAX_ROWS = 100_000


async def _load_dataset_cached(dataset_id: str, user_id: str) -> Optional[pl.DataFrame]:
    """Load dataset from MongoDB with caching. Returns sampled DataFrame if > 100K rows."""
    if dataset_id in _DF_CACHE:
        logger.debug(f"[CACHE] Using cached DataFrame for {dataset_id}")
    ...
    if row_count > QUIS_MAX_ROWS:
        df = df.sample(n=QUIS_MAX_ROWS, seed=42)
        logger.info(f"[SAMPLE] Sampled {QUIS_MAX_ROWS:,} from {row_count:,} rows for QUIS")
...
async def planner_node(state: AgentState) -> Dict[str, Any]:
    ...
    if not state["questions"]:
        try:
            df = await _load_dataset_cached(state["dataset_id"], state["user_id"])
            if df is None:
                return {
...
async def analyst_node(state: AgentState) -> Dict[str, Any]:
    ...
    and state management. Uses belief_context for business rules.
    """
    ...
    belief_ctx = state.get("belief_context", [])
    if belief_ctx:
        belief_str = belief_ctx[0] if isinstance(belief_ctx, list) else str(belief_ctx)
        logger.info(f"[ANALYST] Active beliefs: {belief_str[:100]}...")
    ...
    try:
        ...
        # Enrich question with belief context if available
        enriched_question = current_question["question"]
        if belief_ctx:
            ctx_str = belief_ctx[0] if isinstance(belief_ctx, list) else str(belief_ctx)
            enriched_question = (
    ...
    try:
            ...
            try:
                ...
                if valid_cols:
                    ...
                    if bad_cols:
                        ...
                        suggestions.append(f"Column(s) {bad_cols} not found in dataset schema")
        ...
        return {
            "critique": {
                "score": score,
                "passed": passed,
                "feedback": "Insight validated successfully"
                if passed
...
async def novelty_filter_node(state: AgentState) -> Dict[str, Any]:
    ...
    (
        semantic_surprisal,
        similar_beliefs,
    ) = await belief_store.calculate_semantic_surprisal(
    ...
    if is_novel:
        ...
        logger.info(
            f"[NOVELTY] Filtered as boring. Similar to: {belief_context[0][:50] if belief_context else 'N/A'}..."
        )
...
def route_after_critic(
    state: AgentState,
) -> Literal["analyst", "novelty_filter", "synthesizer"]:
    """
...
async def run_agentic_quis(
    ...
    # Create initial state with belief context
    ...
    if belief_list:
        initial_state["belief_context"] = belief_list
...
async def run_quis_analysis(
    dataset_id: str, user_id: str, query: str = None, novelty_threshold: float = 0.35
) -> Dict[str, Any]:
    """
    ...
    # Load active beliefs for business rule context
    belief_block = ""
    try:
        from services.memory.belief_service import BeliefService

        belief_service = BeliefService(db)
        belief_block = await belief_service.format_for_prompt(user_id, str(dataset_id))
        if belief_block:
            logger.info(f"Loaded beliefs for QUIS analysis: {len(belief_block)} chars")
    except Exception as e:
        logger.warning(f"Failed to load beliefs for QUIS (non-critical): {e}")
