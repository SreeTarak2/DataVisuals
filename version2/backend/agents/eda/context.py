...
class AgentContext:
    ...
    statistical_findings: Dict = field(
        default_factory=dict
    )  # correlations normalized to {column_a, column_b, correlation}
    deep_analysis: Dict = field(
        default_factory=dict
    )  # enhanced_analysis + quis_insights
    ...
    planner_output: Dict = field(
        default_factory=dict
    )  # Agent 1: intent, key_columns, plan
    data_passport: Dict = field(
        default_factory=dict
    )  # Agent 2: structured data summary
    univariate_report: Dict = field(
        default_factory=dict
    )  # Agent 3: per-column insights
    bivariate_report: Dict = field(
        default_factory=dict
    )  # Agent 4: relationships & patterns
    chart_configs: List[Dict] = field(
        default_factory=list
    )  # Agent 5: Plotly-ready configs
    validation_result: Dict = field(
        default_factory=dict
    )  # Agent 6: QA pass/fail + fixes
    ...
    def schema_summary(self, priority_cols: Optional[List[str]] = None) -> str:
        ...
        for col in self.column_metadata:
            if len(ordered) >= cap:
                break
            if col["name"] not in ordered_names:
                ordered.append(col)
        ...
        for col in ordered:
            ...
            lines.append(
                f"  {col['name']} ({dtype}) — {nulls:.1f}% null, {uniq} unique{extra}"
            )
        return "\n".join(lines) + suffix
    ...
    def column_by_name(self, name: str) -> Optional[Dict]:
        ...
        if not hasattr(self, "_col_index"):
            object.__setattr__(
                self, "_col_index", {c["name"]: c for c in self.column_metadata}
            )
