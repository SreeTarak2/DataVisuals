...
# ---------------------------------------------------------
async def _hydrate_blueprint(
    blueprint: Dict[str, Any], dataset_id: str, user_id: str
) -> Dict[str, Any]:
    """
    ...
    dataset_doc = await db.uploads.find_one({"_id": dataset_oid, "user_id": user_id})
    if not dataset_doc or not dataset_doc.get("file_path"):
        logger.warning(
            f"Cannot hydrate blueprint — dataset {dataset_id} not found or has no file_path"
        )
        return blueprint
        ...
        try:
            if ctype == "kpi":
                ...
                if len(safe_columns) < len(columns):
                    df_cols_lower = {
                        str(c).lower().replace(" ", "_"): c for c in df.columns
                    }
                    for orig_col in columns:
                        ...
                        if normalized in df_cols_lower:
                            ...
                            logger.info(
                                f"Fuzzy-matched column '{orig_col}' → '{df_cols_lower[normalized]}'"
                            )
                    safe_columns = list(
                        dict.fromkeys(safe_columns)
                    )  # dedupe preserving order
                if not safe_columns:
                    logger.warning(
                        f"Chart '{comp.get('title')}' has no valid columns. Skipping hydration."
                    )
                    ...
                    if len(safe_columns) == 1:
                        ...
                        if is_numeric:
                            ...
                            if cat_cols:
                                ...
                                logger.info(
                                    f"Auto-added categorical '{cat_cols[0]}' for chart '{comp.get('title')}'"
                                )
                        else:
                            ...
                            if num_cols:
                                ...
                                logger.info(
                                    f"Auto-added numeric '{num_cols[0]}' for chart '{comp.get('title')}'"
                                )
                    if len(safe_columns) < 2:
                        logger.warning(
                            f"Chart '{comp.get('title')}' needs ≥2 columns for {chart_type_str}, only has {safe_columns}. Skipping."
                        )
                        comp["chart_data"] = {"data": [], "layout": {}}
                        hydrated.append(comp)
                        continue
            ...
            logger.error(
                f"Hydration failed for component '{comp.get('title')}' ({ctype}): {e}"
            )
            if ctype == "chart":
                comp["chart_data"] = {"data": [], "layout": {}}
            elif ctype == "kpi":
...
async def retry_single_chart(
    ...
    try:
        ...
        if not cfg:
            raise HTTPException(
                status_code=400, detail="Missing chart config in request body."
            )
        ...
        dataset_doc = await db.uploads.find_one(
            {"_id": dataset_oid, "user_id": user_id}
        )
        if not dataset_doc or not dataset_doc.get("file_path"):
        ...
        if chart_type_str not in flexible_types and len(safe_columns) < 2:
            raise HTTPException(
                status_code=400,
                detail=f"Chart type '{chart_type_str}' needs at least 2 valid columns, but only found: {safe_columns}",
            )

        if not safe_columns:
            raise HTTPException(
                status_code=400,
                detail="No valid columns found in the dataset for this chart.",
            )
        ...
        try:
            ...
            logger.warning(
                f"Failed to update component '{component.get('title')}' config in database: {e}"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to re-hydrate chart: {str(e)}"
        )
...
async def generate_ai_dashboard(
    ...
    """
    return await ai_service.generate_ai_dashboard(
        dataset_id, current_user["id"], force_regenerate
    )
...
async def design_intelligent_dashboard(
    ...
    try:
        ...
        # Invalidate dashboard cache when regenerating to ensure fresh KPIs/charts/insights
        if force_regenerate:
            logger.info(
                f"Redesign requested — invalidating dashboard cache for {dataset_id}"
            )
        ...
        if lock.locked() and not force_regenerate:
            logger.info(
                f"Dashboard generation already in-flight for {lock_key}, waiting for result..."
            )
            async with lock:
                ...
                if existing:
                    ...
                    if blueprint and isinstance(blueprint, dict):
                        logger.info(
                            f"Hydrating cached designer blueprint for waiting request: {dataset_id}..."
                        )
        ...
        raise HTTPException(
            ...
            detail="Failed to design dashboard.",
        )
...
async def get_existing_dashboard(
    ...
    try:
        ...
        if not dashboard:
            raise HTTPException(
                status_code=404,
                detail="No dashboard found for this dataset. Generate one first.",
            )

        blueprint = dashboard.get("dashboard_blueprint")
        if blueprint and isinstance(blueprint, dict):
            logger.info(f"Hydrating cached dashboard for dataset {dataset_id}...")
            dashboard["dashboard_blueprint"] = await _hydrate_blueprint(
                blueprint, dataset_id, current_user["id"]
            )
...
async def generate_data_story(
    ...
    try:
        ...
        return await ai_service.generate_data_story(
            dataset_id, current_user["id"], story_type
        )
    except Exception as e:
        logger.error(f"Error generating data story: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to generate data story.",
        )
...
async def generate_quis_insights(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to generate QUIS insights.",
        )
...
async def explain_chart(
    ...
    try:
        ...
        return await ai_service.explain_chart(
            dataset_id, current_user["id"], chart_config, chart_data
        )
    except Exception as e:
        logger.error(f"Error explaining chart: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to explain chart.",
        )
...
async def generate_business_insights(
    ...
    try:
        ...
        return await ai_service.generate_business_insights(
            dataset_id, current_user["id"], business_context
        )
    except Exception as e:
...
async def run_analysis(
    ...
    try:
        ...
        if not dataset_id or not analysis_type:
            raise HTTPException(
                status_code=400, detail="Dataset ID and analysis type are required"
            )
        ...
        if not callable(analysis_func):
            logger.error(
                f"Analysis method '{method_name}' not found on analysis_service"
            )
