...
class AgenticAnalysisRequest(BaseModel):
    ...
    novelty_threshold: float = Field(
        0.35, ge=0.0, le=1.0, description="Minimum novelty to show (0-1)"
    )
    max_questions: int = Field(
        15, ge=1, le=50, description="Maximum questions to generate"
    )
...
class FeedbackRequest(BaseModel):
    ...
    feedback_type: str = Field(
        ..., pattern="^(useful|known|wrong)$", description="Type of feedback"
    )
...
async def run_agentic_analysis(
    request: AgenticAnalysisRequest, current_user: dict = Depends(get_current_user)
):
    """
    ...
    try:
        # Datasets are stored with _id = str(uuid.uuid4()) — a plain string.
        # Try string _id first, then ObjectId (for any legacy records).
        db = get_database()

        dataset = await db.uploads.find_one({"_id": dataset_id, "user_id": user_id})

        if dataset is None:
            # Legacy fallback: ObjectId-based _id
            try:
...
async def submit_feedback(
    ...
    try:
        ...
        if request.feedback_type == "useful":
            ...
            logger.warning(
                f"User {user_id} marked insight as wrong: {request.insight_text[:100]}..."
            )
            message = "Thank you for the feedback. We'll work on improving accuracy."

        return FeedbackResponse(success=True, belief_id=belief_id, message=message)

    except Exception as e:
        logger.error(f"Feedback submission failed: {e}")
        return FeedbackResponse(
            success=False, message=f"Failed to process feedback: {str(e)}"
        )


@router.get("/beliefs", response_model=BeliefListResponse)
async def list_beliefs(current_user: dict = Depends(get_current_user), limit: int = 50):
    """
    ...
    try:
        ...
        return BeliefListResponse(beliefs=beliefs, total_count=count)
...
async def delete_belief(belief_id: str, current_user: dict = Depends(get_current_user)):
    """
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Belief not found"
            )
...
async def clear_all_beliefs(current_user: dict = Depends(get_current_user)):
    """
    ...
    try:
        ...
        if success:
            return {
                "success": True,
                "message": "All beliefs cleared. Novelty filtering reset.",
            }
        else:
