...
// import DataQualityIndicator from '../components/DataQualityIndicator'; // Removed
...
const Dashboard = () => {
  ...
  const MAX_REDESIGNS = 3; // Limit to 3 redesigns per session
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      ...
      try {
        ...
        // Check if dataset is empty (0 rows or 0 columns) or still processing
        ...
        // Finally load insights (this calls Ollama - do it last with timeout)
        const insightsController = new AbortController();
        const insightsTimeout = setTimeout(() => insightsController.abort(), 10000); // 10 second timeout
        
        let insightsRes;
        try {
          insightsRes = await fetch(`/api/dashboard/${selectedDataset.id}/insights`, {
            headers: { 'Authorization': `Bearer ${token}` },
          ...
          if (fetchError.name === 'AbortError') {
            ...
            insightsRes = { ok: false, status: 408 }; // Timeout status
          } else {
            throw fetchError;
          }
        }
        ...
        if (insightsRes.ok) {
          ...
          console.warn('Insights not loaded (status:', insightsRes.status, ') - Dashboard will render without AI insights');
          setInsights([]);
        }
  ...
  const generateAiDashboard = useCallback(async (forceRegenerate = false) => {
    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (dashboardConfig.dashboard_blueprint && dashboardConfig.dashboard_blueprint.components) {
            ...
            // Toast will be shown after setLayoutLoading(false) in finally block
            return;
      ...
      const wasSuccessful = aiDashboardConfig !== null;
      setLayoutLoading(false);
      
      // Show single success toast only after loading completes
      if (wasSuccessful) {
        // Use setTimeout to ensure state updates are complete
        setTimeout(() => {
          toast.success('✨ AI Dashboard generated successfully!', {
            id: 'dashboard-generated', // Prevent duplicates
            duration: 3000,
          });
        }, 100);
      }
    }
  }, [selectedDataset, aiDashboardConfig]);
  ...
  }, [generateAiDashboard, redesignCount, MAX_REDESIGNS]);
  ...
  // Generate AI dashboard when dataset changes (FIXED: Run only when dataset ID changes, not on every function recreation)
  useEffect(() => {
    if (selectedDataset?.id) {
      generateAiDashboard();
      loadInsights();
    }
  }, [selectedDataset?.id]); // Only re-run when dataset ID actually changes
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-6 space-y-8">
      ...
      ) : selectedDataset.row_count === 0 || selectedDataset.column_count === 0 ? (
        /* Empty Dataset State */
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="flex gap-3">
                ...
                <Button
                  onClick={() => setShowRedesignLimitModal(false)}
