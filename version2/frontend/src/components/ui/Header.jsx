    ...
    return (
        <header
            className={cn('sticky top-0 z-50 w-full border-b border-transparent transition-all duration-300', {
                'bg-[#0A0A0A]/95 supports-[backdrop-filter]:bg-[#0A0A0A]/50 border-white/[0.05] backdrop-blur-lg':
                    scrolled,
            })}
        >
            <nav className="mx-auto flex h-16 w-full max-w-5xl items-center justify-between px-4">
                <div className="flex items-center gap-8">
                    <a href="/" className="hover:bg-white/5 rounded-xl px-3 py-2 flex items-center gap-3 transition-all duration-300">
                    ...
                    <nav className="hidden md:flex items-center gap-8">
                        ...
                        <a href="/#how-it-works" className="text-[11px] uppercase tracking-widest font-bold text-white/60 hover:text-white transition-colors">How it works</a>
                        <a href="/#faq" className="text-[11px] uppercase tracking-widest font-bold text-white/60 hover:text-white transition-colors">FAQ</a>
                    </nav>
                </div>
                <div className="hidden items-center gap-4 md:flex">
                    <Button variant="ghost" asChild className="text-[11px] uppercase tracking-widest font-bold hover:bg-white/5 hover:text-white transition-all">
                        <Link to="/login">Log In</Link>
                    </Button>
                    <Button asChild className="bg-white hover:bg-neutral-200 text-black text-[11px] uppercase tracking-widest font-bold rounded-xl px-6 py-5 shadow-[0_2px_15px_rgba(255,255,255,0.1)] transition-all">
                ...
                <Button
                    ...
                    className="md:hidden border-white/[0.1] bg-white/[0.02] rounded-xl"
                    aria-expanded={open}
            ...
            <MobileMenu open={open} className="flex flex-col justify-between gap-6 overflow-y-auto">
                <nav className="flex flex-col gap-y-6 pt-4 px-2">
                    <a href="/#features" onClick={() => setOpen(false)} className="text-[13px] uppercase tracking-widest font-bold text-white/80 hover:text-white transition-colors">Features</a>
                    <a href="/#how-it-works" onClick={() => setOpen(false)} className="text-[13px] uppercase tracking-widest font-bold text-white/80 hover:text-white transition-colors">How it works</a>
                    <a href="/#faq" onClick={() => setOpen(false)} className="text-[13px] uppercase tracking-widest font-bold text-white/80 hover:text-white transition-colors">FAQ</a>
                </nav>
                <div className="flex flex-col gap-3 pt-6 border-t border-white/[0.05]">
                    <Button variant="outline" asChild className="w-full bg-transparent text-[11px] uppercase tracking-widest font-bold border-white/[0.1] rounded-xl py-6">
                        <Link to="/login">Log In</Link>
                    </Button>
                    <Button asChild className="w-full bg-white hover:bg-neutral-200 text-black text-[11px] uppercase tracking-widest font-bold rounded-xl py-6 shadow-[0_2px_15px_rgba(255,255,255,0.1)] transition-all">
                        <Link to="/register">Get Started</Link>
                    </Button>
                </div>
            </MobileMenu>
